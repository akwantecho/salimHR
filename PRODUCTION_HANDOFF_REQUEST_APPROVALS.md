...
## ⭐ تدفّق نقل المريض = خطوتان (أخصائي ثم أدمن)
1. الأخصائي (أ) يطلب نقل مريض للأخصائي (ب) → الحالة `pending_specialist`.
2. **الأخصائي (ب)** يستقبله في تطبيقه ويقبل/يرفض:
   - **قبول** → الحالة `pending_admin` (يروح للأدمن).
   - **رفض + سبب** → الحالة `rejected`، و**السبب يُرسل إشعاراً للأخصائي (أ) الطالب**.
3. **الأدمن** يعتمد/يرفض النهائي (شاشة الموافقات). الرفض + سبب → إشعار للطالب (أ). القبول → نفّذ النقل.

### مسارات جانب الأخصائي (ب) — جديدة
```php
GET  /api/employee/incoming-transfers         // طلبات موجّهة لي، status=pending_specialist
POST /api/employee/incoming-transfers/{id}/accept   // → pending_admin
POST /api/employee/incoming-transfers/{id}/reject   // body:{reason} → rejected + إشعار للطالب
```
شكل عنصر الوارد:
```json
{ "id": 12, "patient": {"id":8,"name":"..."},
  "requester": {"name":"د. صفاء"},   // الأخصائي الطالب (أ)
  "notes": "سبب النقل", "created_at": "..." }
```
> **مهم:** في `GET /api/approvals` (الأدمن) رجّع في `transfers[]` **فقط** الطلبات `pending_admin`
> (اللي الأخصائي ب قبلها) — مو `pending_specialist`.
> ورفض الأدمن/الأخصائي (ب) دائماً يرسل السبب **للأخصائي الطالب (أ)**.

---

# (تفاصيل اعتماد الأدمن — كما هي)
